...
SpriteSheet::SpriteSheet(sf::Texture& tex, int CellCountHor, int CellCountVert)
{
	...
	cellWidth = tex.getSize().x / CellCountHor; //256, 16
	cellHeight = tex.getSize().y / CellCountVert; //48, 3
...
sf::Sprite SpriteSheet::GetSprite(int i)
{
	sf::Sprite& GlyphSprite = GlyphCollection[i];
